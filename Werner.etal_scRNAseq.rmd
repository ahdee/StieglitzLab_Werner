...
cellmarker [[ "HSC" ]] = c("CD34", "CD38", "KIT", "PROM1"  )
...
# CD123 = "IL3RA"
 ...
 hsc_selection$plot_0 =  hsc_selection$plot_0  +   geom_vline(aes(xintercept = 12), color = "blue", linetype = "dotted") +
  geom_vline(aes(xintercept = hsc_line[1]), color = "blue", linetype = "dotted") +
   ...
   scale_y_discrete(labels = rev ( c("CD34", "CD38", "CD117 (KIT)", "CD133 (PROM1)" ) ) )
...
# remove UMAP labels 
cairo_pdf(paste0(out_figure, "celltype_plot_main_noleg.pdf"), width=8, height=7) 
rasterize(cell_type_plot + theme(legend.position = "none"), layers='Point', dpi=300)
dev.off()


cairo_pdf(paste0(out_figure, "celltype_plot_legend.pdf"), width=8, height=7) 
rasterize(cell_type_plot_multi , layers='Point', dpi=300)
dev.off()
...
* top is a full view of CLEC12A for every cell type while bottom shows only HSC cells. 

```{r fig=TRUE,fig.width=6, fig.height=10, echo=FALSE, include=TRUE, results='asis' }
...
for ( nm in names ( main_deg ) ){
  ...
  scale_color_manual(values = group_color ) + theme( legend.position = "bottom")
   ...
   pathcollect = data.frame()
   pathcollect = rbind ( main_deg[[c]]$hyper$df$KEGG ,main_deg[[c]]$hyper$df$HALLMARK   )
   pathcollect = rbind ( pathcollect, main_deg[[c]]$hyper$df$REACT )
   pathcollect = rbind ( pathcollect, main_deg[[c]]$hyper$df$CC )
   pathcollect = rbind ( pathcollect, main_deg[[c]]$hyper$df$BP )
   pathcollect = rbind ( pathcollect, main_deg[[c]]$hyper$df$MF )
   pathfile = paste0(c,"Pathway")
   addWorksheet(wb, pathfile )
   writeData(wb, pathfile , pathcollect   , rowNames=F  )
...
sf =sessioninfo::session_info(pkgs = "attached")
